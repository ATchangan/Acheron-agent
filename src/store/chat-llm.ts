...
import { updateContextLimit, buildContextualMessages, isVisionModel, outputLimit, estimateTokens, calibrateTokens } from './context'
...
export function createCallLLM(deps: CallLlmDeps): (aid: string, ridArg?: string) => Promise<CallResult> {
  ...
  return (aid: string, ridArg?: string): Promise<CallResult> =>
    new Promise((resolve, reject) => {
      ...
      const msgs = buildContextualMessages(cur.messages, isVisionModel(model), { gSnap, cl: get().cl, spIshiki: get().spIshiki, spFallback: get().sp, agent: cur.agent, onAgentRoute: (role) => { if (role) { set(s => ({ sessions: s.sessions.map(x => x.id === sid ? { ...x, agent: role as string, activeAgents: (x.activeAgents || []).includes(role as string) ? x.activeAgents : [...(x.activeAgents || []), role as string] } : x) })) } } })
      // 更新上下文用量
