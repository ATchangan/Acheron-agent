...
import { Key, SlidersHorizontal, UserRound, Database, Users, Wrench, Puzzle, BookOpen, Palette, BarChart3, Settings as SettingsIcon, Info, Download, Upload, RotateCcw, Activity, Cat } from 'lucide-react'
...
const PetTab = React.lazy(() => import('./settings/PetTab'))
...
export default function SettingsView({ onNavigate }: { onNavigate: (v: string) => void }) {
  ...
  const TABS = [
    ...
    { key: 'pet', icon: <Cat size={15} />, label: '桌宠' },
  ...
  const SETTING_KEYWORDS: [string, string[]][] = [
    ...
    ['pet', ['桌宠', '宠物', '黄泉', '坐视窗', '坐任务栏', '漂浮', '呼吸', '大头', 'Q版', '气泡', '透明', '置顶']],
    ['stats', ['缓存', '统计', '用量', '命中']],
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          <React.Suspense fallback={<div style={{ padding: 24, color: C.muted, fontSize: 'calc(var(--ui-font-size) - 1px)' }}>加载中…</div>}>
            {tab === 'models' ? <ModelsTab showToast={showToast} /> : tab === 'strategy' ? <StrategyTab /> : tab === 'persona' ? <PersonaTab /> : tab === 'memory' ? <MemoryTab /> : tab === 'collab' ? <CollabTab onNavigate={(pg) => onNavigate(pg)} setTab={setTab} openWfModal={(n, d) => { setWfName(n); setWfDesc(d); setWfModal(true) }} /> : tab === 'mcp' ? <McpTab /> : tab === 'skills' ? <SkillsTab /> : tab === 'stats' ? <StatsTab /> : tab === 'diagnostics' ? <DiagnosticsTab /> : tab === 'skin' ? <SkinTab /> : tab === 'pet' ? <PetTab /> : tab === 'tools' ? <ToolsTab /> : tab === 'advanced' ? <AdvancedTab /> : tab === 'cron' ? <CronView /> : tab === 'knowledge' ? <KnowledgeView /> : tab === 'plugins' ? <PluginsView /> : tab === 'about' ? <AboutTab /> : null}
          </React.Suspense>
        </div>
      </div>
