...
export default function McpTab() {
  ...
  const persistServer = (next: McpServerConfig) => {
    const list = Array.isArray(g.mcpServers) ? g.mcpServers : []
    save({ mcpServers: [...list.filter(s => s.name !== next.name), next] })
  }
  const forgetServer = (name: string) => {
    const list = Array.isArray(g.mcpServers) ? g.mcpServers : []
    save({ mcpServers: list.filter(s => s.name !== name) })
  }
  return (
    <div style={U.pageBody}>
      <div style={S.card}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
          ...
          <button style={S.btn('primary')} onClick={async () => { if (!mcpName || !mcpCmd) { showToast('请填写名称和命令'); return } const args = mcpArgs.split(/\s+/).filter(Boolean); const r = await window.huangquan.mcpConnect(mcpName, mcpCmd, args); const ok = typeof r !== 'string'; if (ok) persistServer({ name: mcpName, type: 'stdio', command: mcpCmd, args }); showToast(typeof r === 'string' ? r : ('已连接：' + mcpName)); setMcpName(''); setMcpCmd(''); setMcpArgs(''); window.huangquan.mcpList?.().then((s) => setMcpServers(s || [])) }}>连接</button>
        </div>
        ...
        {mcpServers.length === 0 ? <div style={{ color: C.muted, fontSize: 'calc(var(--ui-font-size) - 2px)' }}>未连接任何 MCP 服务器</div> : mcpServers.map((s, i: number) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderRadius: 6, background: C.input, marginBottom: 6, border: '1px solid ' + C.border }}>
            ...
            <button style={{ ...S.btn('danger'), height: 26, fontSize: 'calc(var(--ui-font-size) - 3px)', padding: '0 10px' }} onClick={async () => { try { await window.huangquan.mcpDisconnect(s.name) } catch { /* 忽略 */ } forgetServer(s.name); showToast('已断开 ' + s.name); window.huangquan.mcpList?.().then((x) => setMcpServers(x || [])) }}>断开</button>
          </div>
        ))}
      </div>
      <div style={S.card}>
        ...
        <div style={U.gap6}>
          ...
          <button style={S.btn('primary')} onClick={async () => { if (!mcpSseName || !mcpSseUrl) { showToast('请填写名称和 URL'); return } const r = await window.huangquan.mcpSSEConnect(mcpSseName, mcpSseUrl); const ok = typeof r !== 'string'; if (ok) persistServer({ name: mcpSseName, type: 'sse', url: mcpSseUrl }); showToast(typeof r === 'string' ? r : ('已连接：' + mcpSseName + '（' + (Array.isArray(r) ? r.length : 0) + ' 工具）')); setMcpSseName(''); setMcpSseUrl('') }}>连接</button>
        </div>
      </div>
      <div style={S.card}>
        <div style={S.section}>MCP 行为配置</div>
        <Toggle checked={g.mcpAutoReconnect !== false} onChange={v => save({ mcpAutoReconnect: v })} label="断线自动重连（本地 stdio 服务器，最多 3 次退避重试）" />
        <Toggle checked={g.mcpAutoConnectOnStart === true} onChange={v => save({ mcpAutoConnectOnStart: v })} label="启动后自动连接已保存的 MCP 服务器" />
