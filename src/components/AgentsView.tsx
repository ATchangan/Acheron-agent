...
type AgentRoleType = '阎罗王' | '判官' | '无常' | '孟婆' | '钟馗'
...
const ROLE_TEMPLATES: RoleTemplate[] = [
  { role: '阎罗王', icon: '👑', description: '黄泉主宰，统筹全局任务分配与决策' },
  { role: '判官',   icon: '📜', description: '文书判官，擅长文档处理、文本分析、内容审核' },
  { role: '无常',   icon: '🔔', description: '黑白无常，负责消息通知、定时提醒、事件监控' },
  { role: '孟婆',   icon: '🌸', description: '忘川摆渡人，专精情感陪伴、心理疏导、日常闲聊' },
  { role: '钟馗',   icon: '⚔️', description: '驱魔天师，负责安全检查、代码审查、风险预警' },
]
...
const AgentsView: React.FC = () => {
  ...
  const [newAgentRole, setNewAgentRole] = useState<AgentRoleType>('阎罗王')
  ...
  const DEFAULT_AGENTS: Agent[] = [
    { id: '__yanluo__', name: '阎罗王', role: '阎罗王', icon: '👑', status: 'idle', description: '黄泉主宰，统筹全局任务分配与决策', taskCount: 0, createdAt: 0 },
    { id: '__panguan__', name: '判官', role: '判官', icon: '📜', status: 'idle', description: '文书判官，擅长文档处理、文本分析、内容审核', taskCount: 0, createdAt: 0 },
    { id: '__wuchang__', name: '无常', role: '无常', icon: '🔔', status: 'idle', description: '黑白无常，负责消息通知、定时提醒、事件监控', taskCount: 0, createdAt: 0 },
    { id: '__mengpo__', name: '孟婆', role: '孟婆', icon: '🌸', status: 'idle', description: '忘川摆渡人，专精情感陪伴、心理疏导、日常闲聊', taskCount: 0, createdAt: 0 },
    { id: '__zhongkui__', name: '钟馗', role: '钟馗', icon: '⚔️', status: 'idle', description: '驱魔天师，负责安全检查、代码审查、风险预警', taskCount: 0, createdAt: 0 },
  ]
  ...
  const startDiscussion = useCallback(() => {
    ...
    const responses: Record<AgentRoleType, string[]> = {
      '阎罗王': [
        `本王以为，「${topic}」此事需从全局着眼。各方各司其职，方能万无一失。`,
        `善。此事交由判官先行梳理文书，无常监察进度，钟馗把关安全，孟婆安抚人心。`,
        `大局已定。诸位依计行事，本王在此坐镇。`,
      ],
      '判官': [
        `关于「${topic}」，我已有初步分析。此事务须条分缕析，逐项核查。`,
        `文书层面我已梳理完毕，关键节点有三：其一为流程合规，其二为数据准确，其三为时效保障。`,
        `案卷已备，请阎罗王过目定夺。`,
      ],
      '无常': [
        `叮～「${topic}」相关动态已纳入监控，有丝毫风吹草动，我即刻来报。`,
        `已设定三个关键时点提醒，确保每个节点都有人跟进。`,
        `报！前方传来新消息，需诸位关注。`,
      ],
      '孟婆': [
        `🌸 关于「${topic}」，我想从情感角度稍作补充——此事关乎人心，不可全以理度之。`,
        `若是涉及人情往来，不妨多一分体谅，少一分苛责。忘川之水，最知人性冷暖。`,
        `诸君议事劳神，不如饮碗汤歇息片刻？`,
      ],
      '钟馗': [
