...
export default function MemoryView() {
  ...
  const load = useCallback(async () => {
    try { setLoading(true); const m = await window.huangquan.memory.load(); setPinnedFacts((m as any).pinnedFacts || []) }
  ...
  const s = {
    container: { padding: '24px 28px', height: '100%', overflowY: 'auto', color: '#E8E8F0', background: 'rgba(13,13,26,0.55)' },
  ...
  return (
    <div style={s.container}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
        ...
        <button style={{...s.btn('#D4AF37'), marginLeft: 0}} onClick={addPinned}>添加</button>
      </div>
      ...
      {pinnedFacts.length === 0 ? <div style={s.empty}>暂无置顶记忆 — 在对话中说"记住xxx"即可写入</div> :
        pinnedFacts.map((f, i) => (
          <div key={i} style={s.card}>
            ...
            <button style={s.btn('#C23B22')} onClick={() => deletePinned(i)}>删除</button>
          </div>
        ))
      }
