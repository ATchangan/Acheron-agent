...
export default function FirstRunOverlay({ onDone, onOpenSettings }: {
  ...
  const steps = [
    { icon: <KeyRound size={18} />, title: '欢迎使用 Acheron-Agent', desc: '本地优先的桌面 AI 助手：会话、记忆、技能、定时任务、插件与多角色协作全部在本地运行。', action: '下一步', run: () => setStep(1) },
    ...
    { icon: <Folder size={18} />, title: '设置工作目录', desc: '配置工作目录后，文件、产物与项目树都会以它为根。配置完成即视为完成引导，之后不再出现。', action: '去设置工作目录', run: () => { onOpenSettings('advanced'); onDone() } },
