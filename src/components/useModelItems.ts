...
export function useModelItems() {
  ...
  const classifyModel = (m: string): 'text' | 'image' | 'video' => {
  ...
  const modelItems: { key: string; label: string; group: 'text' | 'image' | 'video'; pid: string; model: string; isMedia: boolean }[] = []
  ...
  for (const mp of cfgMedia) {
    const push = (ms: string[] | undefined, kind: 'image' | 'video') => (ms || []).forEach(m => modelItems.push({ key: kind + '::' + mp.id + '::' + m, label: m, group: kind, pid: mp.id, model: m, isMedia: true }))
