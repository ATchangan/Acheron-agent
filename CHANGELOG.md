...
- **执行机制与项目集成（本轮）**：
  ...
  - **一键环境自检**：设置 → 诊断 新增「一键环境自检」，覆盖 PowerShell 7/Windows PowerShell/cmd、工作目录、用户数据/会话/回滚目录、API 供应商与网络连通、磁盘/内存、技能、插件、浏览器内核、渲染模式、记忆文件、代理、Git、MCP、本地服务共 21 项，全部只读并给出异常项的解决建议；新增 IPC + 单测
  - **PowerShell 安装场景全覆盖**：检测升级为「PATH 中的 pwsh → 已知安装路径(Program Files\PowerShell\7 与 7-preview) → Windows PowerShell」三级回退，覆盖 winget/MSI/商店/绿色版等其它用户常见安装方式；完整路径自动加引号嵌入命令，编码判断改用能力标志而非字符串比较；自检直接显示 pwsh 版本与安装路径（商店版不在 Program Files，避免误判未安装）；新增 5 条安装场景单测
  - **用户消息重新生成**：自己发送的消息悬停新增圆圈箭头「重新生成回复」按钮（与助手消息同款），点击后从该条消息截断历史重新生成回复；执行中禁用防误触
...
验证：`npm run build` [OK] / vitest 全绿 [OK] / 6 套预设主题逐一截图验证 [OK] / CDP 实测（浏览器实时画面、消息区样式对齐、风险卡片、多轮记忆）[OK]
...
验证：`npm run build` [OK] / vitest 17 文件 95 用例全绿 [OK] / CDP 实测（多轮记忆、工具调用、插话原文、停止 0 重试、风险卡片、原生窗口按钮、真实游戏任务完整交付）[OK]
