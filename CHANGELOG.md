...
- **执行机制与项目集成（本轮）**：
  ...
  - **计划一致性修复**：`update_plan` 步骤无 id 时按 label+tool 合并，重复调用不再产生一模一样的两条；计划步骤按 id 与“未认领 pending 同 label+tool”双重去重（引擎与渲染层增量合并都加防御）；每任务技能清单只扫描一次（不再每轮读盘）；新增回归测试（连续两次相同 update_plan 后计划仍为 2 步）
