...
- **执行机制与项目集成（本轮）**：
  ...
  - **项目指令升级（混合主流方案）**：发现语义对齐 Codex——每目录按 `AGENTS.override.md` > `AGENTS.md` > `CLAUDE.md` > `.agents.md` 解析，git 根 → 工作目录逐层合并、深层靠后优先，合并上限默认 32 KiB（设置 → 引擎 → 项目指令上限可调），超限打截断标记不再静默丢内容；注入策略对齐 Hermes——启动只注入工作目录链，模型读取子目录文件时按需注入该目录（上溯最多 5 层）的规则（单文件 8k 上限、每会话每目录一次）；写作纪律对齐 Claude——系统提示注明常驻文件建议 200 行内、重内容拆子目录；所有注入前做提示注入安全扫描，可疑文件跳过
