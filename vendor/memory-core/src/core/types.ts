 ...
 * and the host environment (OpenClaw, Agent, standalone Gateway, etc.).
 *
 * Design principles:
 ...
 * In Agent:   populated from `MemoryProvider.initialize()` kwargs.
...
export interface RuntimeContext {
  ...
  platform: "openclaw" | "agent" | "cli" | "gateway" | string;
 ...
 * - `StandaloneLLMRunner`: direct OpenAI-compatible HTTP calls (Gateway / Agent host)
 */
 ...
 * - Agent/GW:   `StandaloneHostAdapter` — wraps Gateway HTTP request context
 *
...
export interface HostAdapter {
  ...
  readonly hostType: "openclaw" | "agent" | "standalone";
