...
#
# 在 install_agent_ubuntu.sh 之后执行，用于：
...
#   3. 配置 agent config.yaml 使用 memory_tencentdb 记忆提供者
...
#
# 前置条件：
#   - install_agent_ubuntu.sh 已执行完成（agent-agent 已安装）
#   - Node.js >= 22 已安装
...
# Agent 路径
AGENT_HOME="$USER_HOME/.agent"
# AGENT_AGENT_DIR（fix: issue #18）
# 用户通过环境变量传什么就用什么；未设置时 fallback 到传统路径。
# 如果目录不存在，后续前置检查会统一报错。
AGENT_AGENT_DIR="${AGENT_AGENT_DIR:-$AGENT_HOME/agent-agent}"
AGENT_CONFIG="$AGENT_HOME/config.yaml"
...
# ==================== root → 自动切换到目标用户 ====================
# 与 install_agent_ubuntu.sh 保持一致：如果以 root 执行且目标用户不是
...
if [ "$(id -u)" -eq 0 ] && [ "$USERNAME" != "root" ]; then
    ...
    if [ ! -d "$AGENT_AGENT_DIR" ]; then
        echo "[ERROR] Agent agent not found at $AGENT_AGENT_DIR"
        echo "[ERROR] Please run install_agent_ubuntu.sh first."
        exit 1
    fi
...
# 加载 agent 环境（PATH 中需要 node/npm）
if [ -f /etc/profile.d/agent-env.sh ]; then
...
# ---------- Step 2.5: 将插件链接到 agent 插件目录 ----------

echo "[memory-tencentdb] Step 2.5: Linking plugin into agent plugins directory..."

AGENT_PLUGIN_DIR="$AGENT_AGENT_DIR/plugins/memory/memory_tencentdb"
PLUGIN_SRC_DIR="$TDAI_INSTALL_DIR/agent-plugin/memory/memory_tencentdb"

# 移除旧链接/目录
rm -rf "$AGENT_PLUGIN_DIR"

# 创建 symlink 使 agent 能发现插件
ln -sf "$PLUGIN_SRC_DIR" "$AGENT_PLUGIN_DIR"

echo "[memory-tencentdb] Plugin linked: $AGENT_PLUGIN_DIR -> $PLUGIN_SRC_DIR"

# ---------- Step 3: 提示用户手动开启 memory_tencentdb（不自动修改 config） ----------

echo "[memory-tencentdb] Step 3: Checking agent config..."

# 插件已链接到 agent 插件目录，但默认不自动启用，仅提示
if [ -f "$AGENT_CONFIG" ]; then
    if sed -n '/^memory:/,/^[a-zA-Z]/p' "$AGENT_CONFIG" | grep -q "provider: memory_tencentdb"; then
        ...
        echo "[memory-tencentdb] To enable tdai memory, add/edit in $AGENT_CONFIG:"
        echo ""
        echo "    memory:"
    ...
    echo "[memory-tencentdb] WARN: $AGENT_CONFIG not found, please run install_agent_ubuntu.sh first"
fi
...
# 当 Agent 或独立 Gateway 以 systemd service 运行时，systemd 不会
...
# 写入 /etc/profile.d 持久化环境变量，供 SSH 手动执行 `agent` 时使用。
...
# ── 4b: ~/.agent/.env（systemd service 场景） ──
# agent-gateway 通过 systemd user service 启动时不会 source /etc/profile.d/*.sh，
# 但 agent 的 run.py 启动时会 load_dotenv("~/.agent/.env")。
...
_append_or_update_env "MEMORY_TENCENTDB_GATEWAY_CMD" "$GATEWAY_CMD" "$AGENT_ENV"
_append_or_update_env "MEMORY_TENCENTDB_GATEWAY_HOST" "127.0.0.1"   "$AGENT_ENV"
_append_or_update_env "MEMORY_TENCENTDB_GATEWAY_PORT" "8420"         "$AGENT_ENV"

echo "[memory-tencentdb] Gateway env vars also written to $AGENT_ENV (for systemd service)"
...
# 验证 agent 插件文件存在（在解压目录中）
PLUGIN_SRC="$TDAI_INSTALL_DIR/agent-plugin/memory/memory_tencentdb"
MISSING=0
...
if [ "$MISSING" -eq 0 ]; then
    echo "  [OK] All agent plugin files present"
fi
...
echo ""
echo "  NOTE: Before using the memory plugin, configure LLM credentials in ~/.agent/.env:"
...
echo ""
echo "  (For systemd-managed agent-gateway, ~/.agent/.env is the authoritative config."
