...
MemoryCore runs independently and exposes these capabilities through an HTTP Gateway. OpenClaw, Agent, and custom applications connect through lightweight adapters or SDKs. An Agent is a caller and a managed metadata entity; MemoryCore does not host, schedule, or execute the Agent itself.
...
```text
OpenClaw / Agent / Custom Application
                  │
                  │ HTTP API / SDK
                  ▼
...
`agent-plugin/` provides the Agent Memory Provider. It follows the same adapter model and uses the Gateway for conversation capture and memory recall.
...
├── agent-plugin/         Agent Memory Provider
