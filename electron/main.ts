...
ipcMain.handle('settings:load', () => {
  try { return fs.existsSync(settingsPath) ? JSON.parse(fs.readFileSync(settingsPath, 'utf-8')) : { providers: [], general: { theme: 'dark' } } }
  catch { return { providers: [], general: { theme: 'dark' } } }
})
...
ipcMain.handle('llm:chat', async (event, params: any) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      event.sender.send('llm:error', `API ${res.status}: ${errBody.slice(0, 300)}`); return
    }
    const reader = res.body?.getReader()
    if (!reader) { event.sender.send('llm:error', '无流'); return }
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        const d = t.slice(6); if (d === '[DONE]') { event.sender.send('llm:chunk', { content: '', done: true }); continue }
        try {
          const p = JSON.parse(d), choice = p.choices?.[0]
          if (choice?.delta?.tool_calls?.[0]) event.sender.send('llm:toolCall', choice.delta.tool_calls[0])
          if (choice?.finish_reason === 'tool_calls') event.sender.send('llm:toolCallDone', { finish_reason: 'tool_calls' })
          const c = choice?.delta?.content || ''
          if (choice?.finish_reason) event.sender.send('llm:chunk', { content: '', done: true })
          else if (c) event.sender.send('llm:chunk', { content: c, done: false })
        } catch { /* */ }
      }
    }
