...
function extractPage(): { title: string; text: string } {
  try {
    ...
    return { title: '', text: 'extract-error: ' + String(e && e.message || e) }
  }
}
...
export async function webRead(rawOpts: WebReadOpts): Promise<WebReadResult> {
  ...
  try {
    ...
    return { ok: false, error: 'Playwright 内核未安装: ' + (e?.message || e), advice: '请执行 npm install playwright-core' }
  }
  ...
  try {
    ...
    if (opts.cookies && opts.cookies.trim()) {
      try {
        ...
        const normalized = cookieList.map(function (c: any) {
          const out: any = { name: c.name, value: c.value, path: c.path || '/', domain: c.domain || host, url: c.url }
          if (c.url) { delete out.domain }
          return out
        })
        await ctx.addCookies(normalized)
      } catch { /* cookie 解析失败则忽略, 不影响主流程 */ }
    }
    ...
    if (mode === 'text') {
      const ex = await page.evaluate(extractPage).catch((e: any) => ({ title: '', text: 'evaluate-error: ' + String(e?.message || e) }))
      out.title = ex.title || ''
      out.text = (ex.text || '')
