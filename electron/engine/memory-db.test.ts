...
describe('SQLite 记忆主路径', () => {
  it('save/load 全字段往返 + 事实/置顶/摘要按内容 reconcile', () => {
    ...
    expect(saveMemory(memoryPath, base, { agent: '黄泉', scope: 'global' })).toBe(true)
    let loaded = loadMemory(memoryPath, { agent: '黄泉', scope: 'global' })
    ...
    expect(saveMemory(memoryPath, { ...base, facts: ['事实B', '事实C'] }, { agent: '黄泉', scope: 'global' })).toBe(true)
    loaded = loadMemory(memoryPath, { agent: '黄泉', scope: 'global' })
    expect(loaded.facts).toEqual(['事实B', '事实C'])
  })

  it('全局/私有记忆按 agent+scope 隔离, FTS/向量检索只命中本作用域', () => {
    saveMemory(memoryPath, { facts: ['全局事实甲'], summaries: [] }, { agent: '黄泉', scope: 'global' })
    saveMemory(memoryPath, { facts: ['私有事实乙'], summaries: [] }, { agent: '银狼', scope: 'private' })
    expect(searchFts('全局事实甲', 10, { agent: '黄泉', scope: 'global' }).length).toBe(1)
    expect(searchFts('私有事实乙', 10, { agent: '黄泉', scope: 'global' })).toHaveLength(0)
    expect(searchFts('私有事实乙', 10, { agent: '银狼', scope: 'private' }).length).toBe(1)
  })

  it('upsertFactDb 同事实去重并累计置信度', () => {
    const id1 = upsertFactDb('黄泉', 'global', '用户喜欢咖啡', false)
    const id2 = upsertFactDb('黄泉', 'global', '用户喜欢咖啡', false)
    expect(id1).toBeGreaterThan(0)
    expect(id2).toBe(id1)
    const rows = listMemories({ agent: '黄泉', scope: 'global', layer: 'L1' }).filter(m => m.content === '用户喜欢咖啡')
    expect(rows.length).toBe(1)
    expect(rows[0].confidence).toBe(2)
  })

  it('recallMemoryDb 无嵌入配置时仍走 FTS 关键词路', async () => {
    saveMemory(memoryPath, { facts: ['项目排期阻塞在本周'], summaries: [] }, { agent: '黄泉', scope: 'global' })
    const hits = await recallMemoryDb('黄泉', 'global', '排期阻塞', 5)
  ...
  it('只带部分字段的保存不会清空缺失字段(防误清库)', () => {
    ...
    }, { agent: '黄泉', scope: 'global' })
    saveMemory(memoryPath, { facts: ['事实B'], summaries: [] } as EngineMemory, { agent: '黄泉', scope: 'global' })
    const loaded = loadMemory(memoryPath, { agent: '黄泉', scope: 'global' })
