...
export const TOOLS: EngineToolSpec[] = [
  ...
  { type: 'function', function: { name: 'memory_search', description: 'memory_search(query, limit?) 检索长期记忆(历史任务沉淀的事实/偏好/约定/决策), 返回相关记忆片段。涉及用户过往上下文的问题先检索', parameters: { type: 'object', properties: { query: { type: 'string' }, limit: { type: 'number' } }, required: ['query'] } } },
  { type: 'function', function: { name: 'conversation_search', description: 'conversation_search(query, limit?) 跨会话检索历史对话原文(谁说过什么、之前怎么做的)', parameters: { type: 'object', properties: { query: { type: 'string' }, limit: { type: 'number' } }, required: ['query'] } } },
  { type: 'function', function: { name: 'skill_search', description: 'skill_search(query, top_k?) 检索可复用技能(从历史成功经验沉淀的流程/方法), 返回技能名与要点', parameters: { type: 'object', properties: { query: { type: 'string' }, top_k: { type: 'number' } }, required: ['query'] } } },
