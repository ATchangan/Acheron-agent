...
## Token 优化（0.3.2~0.3.5）

全系列目标：**不降智力、不降质量**的前提下降低每次 LLM 请求消耗。系列完成后闲聊单请求省 70%+、复杂任务省 50%+（以基准报告实测为准）。
...
| 0.3.3 | 历史图片降级 | 旧轮次图片降级文字，最新消息保留 | imgDowngrade |
| 0.3.3 | 工具参数截断 | 超长参数截断，定位字段保留 | argSlim |
| 0.3.3 | 跨任务归档 | 完成任务折叠为归档记录 | taskArchive |
| 0.3.4 | 插话合并 | 多段插话合并单条注入 | interjectMerge |
| 0.3.5 | 并行结果护栏 | 并行结果总量超限自动瘦身 | parallelCap |
| 0.3.4 | 估算校准 | 分层估算 + 按模型 EMA 实测校准 | 内部机制，无开关 |
...
- 收益基准：`node scripts/token-baseline.mjs`（三档压缩阈值 + 0.3.0 vs 0.3.5 对比）
