...
## Token 优化（v0.3.2 内置，原 0.3.2~0.3.5 系列合并）
...
| 版本 | 优化 | 说明 | 开关 |
|---|---|---|---|
| 0.3.2 | 工具白名单注入 | 按 Agent 只注入其领域工具 | 设置→高级→性能优化→工具白名单注入 |
| 0.3.2 | 工具 schema 中文精简 | 描述精简，参数/安全限制保留 | 内置，无开关 |
| 0.3.2 | 结果瘦身 | >1500 字符截断保留头尾+关键行 | resultSlim |
| 0.3.2 | 记忆按需裁剪 | 相关度 top5 + 总量护栏 2500 | memoryTrim |
| 0.3.2 | 工作流按需注入 | 命中触发词才注入完整列表 | workflowLazy |
| 0.3.2 | 历史轮次折叠 | 旧工具轮次折叠为归档摘要 | roundFold |
| 0.3.2 | 输出上限分级 | 闲聊短消息上限 800 | outputCap |
| 0.3.2 | 历史图片降级 | 旧轮次图片降级文字，最新消息保留 | imgDowngrade |
| 0.3.2 | 工具参数截断 | 超长参数截断，定位字段保留 | argSlim |
| 0.3.2 | 跨任务归档 | 完成任务折叠为归档记录 | taskArchive |
| 0.3.2 | 插话合并 | 多段插话合并单条注入 | interjectMerge |
| 0.3.2 | 并行结果护栏 | 并行结果总量超限自动瘦身 | parallelCap |
| 0.3.2 | 估算校准 | 分层估算 + 按模型 EMA 实测校准 | 内部机制，无开关 |

### 关键参数

- 结果瘦身：>1500 字符，头尾 800/500 + 关键行
- 轮次折叠：>8 对完整轮次，折叠最旧 4 对
- 归档条件：任务块 ≥6 消息 + ≥2 次工具调用 + ≥2 块，最多保留 5 条
- 压缩阈值：模型窗口 70%（可 A/B 调整，见脚本）
- 并行护栏：单轮并行结果 >6000 字符且 >4 个时，保留前 4 个全量
- 估算系数：按模型隔离，EMA 平滑，限幅 0.3~3

### 缓存友好约定

- system prompt 头部（身份/人格/纪律）内容与顺序字节级不变
- 动态内容（记忆/归档/时间戳）只允许追加尾部
- 工具 schema 注入保持 TOOLS 原序（filter 保序，禁止排序/去重）
- Agent 切换（handoff）后首请求缓存失效属正常
- 验证：`node scripts/check-prefix-stable.mjs`

### 基准与门禁

- 智力基线：`docs/baseline-intelligence-report.md`（9 任务 + 10 工具 + 长会话 + 保真）
- 收益基准：`node scripts/token-baseline.mjs`（三档压缩阈值 + 0.3.0 vs 0.3.2 对比）
- 发布门禁：工具选择一致率 ≥95% / 完成度 100% / 保真零丢失 / 回归零异常 / 单点可回退
...
**Token 优化系列合并版（原 0.3.2~0.3.5）**
- 注入层：工具白名单/结果瘦身/记忆裁剪/工作流按需/轮次折叠/输出分级/会话统计/静态瘦身
- 上下文层：历史图片降级/工具参数截断/跨任务归档
- 度量层：分层估算 + EMA 校准/前缀缓存断言/插话合并/压缩阈值基准
- 可控层：并行结果护栏 + 11 开关集中管理（默认全开、单点回退）
- Hermes/Codex 吸收：记忆安全扫描/冻结快照/AGENTS.md/会话搜索/回复去重；关于页版本号动态化
- 界面与模型：全中文化去 AI 味（藏书阁/式神等）、模型分页/全选/自动探测、视觉模型补全
- 调整：移除符文工坊；清理调试残留
