...
- 🛡️ 全局崩溃捕获：主进程异常不再直接退出，记录 crash.log
- 🖥️ 禁用 GPU 硬件加速，防止 GPU 进程崩溃导致窗口渲染异常
- 🔄 AbortController 替代全局标志位，支持并发请求
- 🔧 多工具调用支持（累积工具参数片段）
- 💾 非流式单次 LLM 调用：多 Agent 分发时子 Agent 独立执行
- 🖼️ 视觉辅助模型接口：主模型不支持多模态时自动降级
- 🎛️ 多媒体能力查询（media_describe-options）
- 📤 导出对话历史（md/json/txt）到工作目录
- 🗑️ 清空全部对话历史 + 恢复出厂设置
- 🪟 最小化/关闭缩至托盘设置
- 🔍 更可靠的 PowerShell 检测 + UTF-8 多字节截断修复
- 📏 图片大小限制 20MB，目录扫描深度/文件数限制
- 🐛 修复非 DeepSeek Provider 全部报“不支持的 Provider”
- 🐛 兼容 baseUrl 已含 /v1 或 /v4 的情况
- 💾 真实存储统计 + 写操作时同步失效缓存
